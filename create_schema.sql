...
start abilities (id serial primary key, name varchar(30) not null, class varchar(255) not null, description varchar(255) not null, damage int not null, status varchar(30) not null, mana_cost int not null, range int not null, effect_duration int not null, use_pattern_id int not null, constraint pk_use_pattern foreign key (use_pattern_id) references use_patterns(id) on update cascade on delete cascade)
end abilities
start monsters (id serial primary key, name varchar(30) not null, class varchar(255) not null, attack int not null, defense int not null, health int not null, vision int, movement int not null,rarity_id int not null, constraint pk_rarity foreign key (rarity_id) references rarities(id) on update cascade on delete cascade)
end monsters
start traps (id serial primary key, name varchar(30) not null, class varchar(255) not null, damage int not null, trigger_radius int not null, rarity_id int not null, constraint pk_rarity foreign key (rarity_id) references rarities(id) on update cascade on delete cascade)
end traps
start maps (id serial primary key, map_name varchar(30) not null, creator_id int not null, width int not null, depth int not null, hero_capacity int not null, constraint pk_player_id foreign key (creator_id) references players(id) on update cascade)
end maps
...
start  monster_player (player_id int not null, monster_id int not null, quantity int not null default 0, constraint pk_player_id foreign key (player_id) references players(id) on update cascade on delete cascade, constraint pk_monster_id foreign key (monster_id) references monsters(id) on update cascade on delete cascade)
end  monster_player
start  trap_player (player_id int not null, trap_id int not null, quantity int not null default 0, constraint pk_player_id foreign key (player_id) references players(id) on update cascade on delete cascade, constraint pk_trap_id foreign key (trap_id) references traps(id) on update cascade on delete cascade)
end  trap_player
...
start  ability_hero (ability_id int not null, hero_id int not null, constraint pk_hero_id foreign key (hero_id) references heroes(id) on update cascade on delete cascade, constraint pk_ability_id foreign key (ability_id) references abilities(id) on update cascade on delete cascade)
end  ability_hero
start  ability_monster (ability_id int not null, monster_id int not null, constraint pk_monster_id foreign key (monster_id) references monsters(id) on update cascade on delete cascade, constraint pk_ability_id foreign key (ability_id) references abilities(id) on update cascade on delete cascade)
end  ability_monster
