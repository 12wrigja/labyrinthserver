...
create table use_patterns (id serial primary key, inputs int default 1, rotatable boolean default true);
create table use_pattern_tiles (pattern_id int, x int, y int, damage int, primary key (pattern_id, x, y));
create table hero_items (id serial primary key, name varchar(30) not null, class varchar(255) not null, type varchar(255) not null, attack_change int not null, defense_change int not null, health_change int not null, movement_change int not null, vision_change int not null, use_pattern int not null, use_range int default 0, rarity int not null, constraint pk_rarity foreign key (rarity) references rarities(id) on update cascade on delete cascade, constraint pk_use_pattern foreign key (use_pattern) references use_patterns(id) on update cascade on delete cascade);
create table heroes (id serial primary key, class varchar(255) not null, attack int not null, defense int not null, health int not null, vision int not null, movement int not null, default_weapon int not null, constraint pk_default_weapon foreign key (default_weapon) references hero_items(id));
...
create table levels (hero_id int, experience int, level int, upgrade varchar(255) not null, constraint pk_hero_id foreign key (hero_id) references heroes(id) on update cascade on delete cascade, primary key (hero_id, level));
...
create table hero_player (player_id int not null, hero_id int not null, level int not null default 1, weapon_id int not null, equipment_id int not null, constraint pk_player_id foreign key (player_id) references players (id) on update cascade on delete cascade, constraint pk_hero_id foreign key (hero_id) references heroes (id) on update cascade on delete cascade, constraint pk_weapon_id foreign key (weapon_id) references hero_items(id) on update cascade on delete cascade,constraint pk_equipment_id foreign key (equipment_id) references hero_items(id) on update cascade on delete cascade);
