...
public class PostgresHeroRepository extends DBRepository implements HeroRepository {
    ...
    private static final String GET_HERO_TYPE_QUERY = "select class from " + HEROES_TABLE + " where id = ?";
    ...
    public List<LevelReward> getLevelRewards(HeroType type, long previousExperience, long newExperience) {
        ...
        try {
            ...
            while(rslts.next()){
                ...
                Optional<LevelReward> reward = buildReward(type,levelAwarded,experienceThreshold,rewardStr);
                if(reward.isPresent()){
    ...
    public InternalResponseObject<HeroType> getHeroTypeForId(int id) {
        try {
            PreparedStatement stmt = conn.prepareStatement(GET_HERO_TYPE_QUERY);
            stmt.setInt(1, id);
            ResultSet set = stmt.executeQuery();
            set.next();
            String className = set.getString("class").toUpperCase();
            HeroType type = HeroType.valueOf(className);
            return new InternalResponseObject<>(type, Hero.HERO_TYPE_KEY);
        } catch (SQLException e) {
            if (GameEngine.instance().IS_DEBUG_MODE) {
                e.printStackTrace();
            }
