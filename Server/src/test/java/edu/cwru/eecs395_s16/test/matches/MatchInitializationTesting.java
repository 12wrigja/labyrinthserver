...
public class MatchInitializationTesting extends InMatchTest {
    ...
    public void testNormalMatchInitialization() {
        Set<UUID> heroUUIDs = heroBot.getBotsHeroes().stream().map(GameObject::getGameObjectID).collect(Collectors.toSet());
        ...
        List<Hero> heroes = heroChars.stream().filter(obj->obj instanceof Hero).map(obj->(Hero)obj).collect(Collectors.toList());
    ...
    public void testInitializationMonstersRandom(){
        ...
        Map<Integer,Integer> idToCountMap = architectBot.getArchitectObjects().stream().filter(obj->obj instanceof Monster).map(obj->(Monster)obj).collect(Collectors.groupingBy(Creature::getDatabaseID)).entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry->entry.getValue().size()));
        ...
        List<Monster> monsters = architectChars.stream().filter(obj->obj instanceof Monster).map(obj->(Monster)obj).collect(Collectors.toList());

        Map<Integer,Integer> actualCountMap = new HashMap<>();
        for(Monster monster : monsters){
            actualCountMap.put(monster.getDatabaseID(),actualCountMap.containsKey(monster.getDatabaseID())?actualCountMap.get(monster.getDatabaseID())+1:1);
        }
    ...
    public void testInitializationWhenPlacingMonster(){
        ...
        List<Monster> archMonsters = architectBot.getArchitectObjects().stream().filter(obj->obj instanceof Monster).map(obj->(Monster)obj).collect(Collectors.toList());
    ...
    public void testInitializationWhenPlacingMonsters(){
        ...
        mb = new MonsterBuilder(UUID.randomUUID(), rangedGoblinDef, architectBot.getUsername(), Optional.of(architectBot.getUsername()));
        mb.setWeapon(w);
        initialArchitectObjects.add(mb.createMonster());
        useMonsterIDs = new HashMap<>();
        int spawnLocIndex = 0;
    ...
    public void testTooManyOfAMonsterSpecified(){
        InternalResponseObject<List<MonsterDefinition>> archDefs = GameEngine.instance().services.monsterRepository.getPlayerMonsterTypes(architectBot);
    ...
    public void testNotOwnedMonsterSpecified(){
        ...
        Optional<MonsterDefinition> monsterDef = archDefs.get().stream().filter(def->def.id == notOwnedDefinitionID).findFirst();
    ...
    public void testInvalidLocationSpecified(){
        ...
        Optional<Monster> archMonster = architectBot.getArchitectObjects().stream().filter(obj->obj instanceof Monster).map(obj->(Monster)obj).findAny();
