...
public abstract class InMatchTest extends SerializationTest {
    ...
    public InternalResponseObject<Match> setupMatch(boolean shouldAutoFail) {
        ...
        InternalResponseObject<Match> matchOpt = Match.InitNewMatch(heroBot, architectBot, initialGameMap,
                initialObjective, useHeroIDs, useMonsterIDs);
    ...
    public InternalResponseObject<Boolean> moveCharacter(Player p, UUID characterID, List<Location> path, boolean
            failTestOnFailure) {
    ...
    public InternalResponseObject<Boolean> passCharacter(Player p, Creature character, boolean failTestOnFailure) {
        ...
        updateMatchState(p);
        Creature myCharacter = (Creature) currentMatchState.getBoardObjects().getByID(character.getGameObjectID())
                .get();
    ...
    public InternalResponseObject<Boolean> basicAttackWithCharacter(Player p, UUID characterID, List<Location>
            inputs, boolean failTestOnFailure) {
        ...
        List<Creature> playerCreatures = currentMatchState.getBoardObjects().getForPlayerOwner(p).stream().filter(obj
                -> obj instanceof Creature && ((Creature) obj).getActionPoints() > 0).map(obj -> (Creature) obj)
                .collect(Collectors.toList());
        if (playerCreatures.size() == 1 && playerCreatures.get(0).equals(character) && character.getActionPoints() ==
                1) {
        ...
        for (Location input : inputs) {
            ...
            for (Map.Entry<Location, Float> entry : hitmap.entrySet()) {
                List<Creature> creatures = currentMatchState.getBoardObjects().getForLocation(entry.getKey()).stream
                        ().filter(obj -> obj instanceof Creature).map(tmp -> (Creature) tmp).collect(Collectors
                        .toList());
                for (Creature c : creatures) {
                    int baseDamage = (int) Math.floor(((100 - c.getDefense()) / 100f) * character.getAttack());
                    affectedCreatureDamageMap.put(c, Math.max(0, c.getHealth() - (int) ((float) baseDamage * entry
                            .getValue())));
                }
            }
        }
        ...
        if (failTestOnFailure) {
            for (Map.Entry<Creature, Integer> affectedCreature : affectedCreatureDamageMap.entrySet()) {
                Optional<GameObject> cResp = currentMatchState.getBoardObjects().getByID(affectedCreature.getKey()
                        .getGameObjectID());
                if (cResp.isPresent()) {
