...
public class BasicAttackTestingArchitect extends InMatchTest {
    ...
    public void testAttackingOutOfRange() {
        ...
        InternalResponseObject<Boolean> resp = basicAttackWithCharacter(architectBot, creature.getGameObjectID(),
                inputs, false);
    ...
    public void testAttackingFriendlyCharacter() {
        ...
        MonsterDefinition goblinDef = GameEngine.instance().services.monsterRepository.getMonsterDefinitionForId(1)
                .get();
        initialArchitectObjects.add(new MonsterBuilder(UUID.randomUUID(), goblinDef, architectBot.getUsername(),
                Optional.of(architectBot.getUsername())).createMonster());
        initialArchitectObjects.add(new MonsterBuilder(UUID.randomUUID(), goblinDef, architectBot.getUsername(),
                Optional.of(architectBot.getUsername())).createMonster());
        setupMatch(true);
        ...
        InternalResponseObject<Boolean> resp = basicAttackWithCharacter(architectBot, monster1.getGameObjectID(),
                inputs, false);
        assertFalse(resp.isNormal());
    ...
    public void testAttackSelf() {
        ...
        InternalResponseObject<Boolean> resp = basicAttackWithCharacter(architectBot, monster.getGameObjectID(),
                inputs, false);
        assertFalse(resp.isNormal());
        assertEquals(InternalErrorCode.FRIENDLY_FIRE, resp.getInternalErrorCode());
    }
    ...
    public void testAttackWithEnemyUnit() {
        ...
        List<Location> inputs = new ArrayList<>();
        inputs.add(new Location(1, 3));
        InternalResponseObject<Boolean> resp = basicAttackWithCharacter(architectBot, hero.getGameObjectID(), inputs,
                false);
        assertFalse(resp.isNormal());
    ...
    public void testAttackWithTooFewInputs() {
        ...
        initialArchitectObjects = new ArrayList<>();
        MonsterDefinition dualShotGoblin = new WeaponModMonsterDefinition(-1, GameEngine.instance().services
                .monsterRepository.getMonsterDefinitionForId(1).get(), 1, w.getDatabaseID());
        MonsterBuilder mb = new MonsterBuilder(UUID.randomUUID(), dualShotGoblin, architectBot.getUsername(),
